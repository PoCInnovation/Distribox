...
export default function OverviewPage() {
  ...
  const handleStartVM = async (vm: VM, e?: React.MouseEvent) => {
    ...
    setVms((prev) =>
      prev.map((v) =>
        v.id === vm.id ? { ...v, status: "running" as const, uptime: "0h 0m", cpuUsage: 15, ramUsage: 25 } : v,
      ),
    )
  ...
  const handleStopVM = async (vm: VM, e?: React.MouseEvent) => {
    ...
    setVms((prev) =>
      prev.map((v) =>
        v.id === vm.id ? { ...v, status: "stopped" as const, uptime: "0h", cpuUsage: 0, ramUsage: 0 } : v,
      ),
    )
  ...
  const handleRestartVM = async (vm: VM, e?: React.MouseEvent) => {
    ...
    setVms((prev) => prev.map((v) => (v.id === vm.id ? { ...v, uptime: "0h 0m" } : v)))
  ...
  return (
    <div className="h-full p-8">
      <div className="mb-8">
        <h1 className="mb-2 font-mono text-3xl font-bold tracking-tight text-balance">Dashboard Overview</h1>
        <p className="text-muted-foreground">Monitor and manage your virtual machines</p>
      </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <Card key={stat.label} className="border-border bg-card p-6 transition-all hover:border-primary/50">
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <DialogTitle className="font-mono">{selectedVM.name}</DialogTitle>
              ...
              <Tabs defaultValue="overview" className="mt-4">
                ...
                <TabsContent value="overview" className="space-y-4">
                  <div className="grid gap-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant={selectedVM.status === "running" ? "default" : "secondary"}
                        className={
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-sm">{selectedVM.uptime}</span>
                    </div>
                    <div className="flex items-center justify-between">
                ...
                <TabsContent value="metrics" className="space-y-6">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-sm">{selectedVM.cpuUsage}%</span>
                    </div>
                    <Progress value={selectedVM.cpuUsage} />
                    <p className="text-xs text-muted-foreground">{selectedVM.cpu} vCPU allocated</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-sm">{selectedVM.ramUsage}%</span>
                    </div>
                    <Progress value={selectedVM.ramUsage} />
                    <p className="text-xs text-muted-foreground">{selectedVM.ram} GB allocated</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-sm">{selectedVM.diskUsage}%</span>
                    </div>
                    <Progress value={selectedVM.diskUsage} />
                    <p className="text-xs text-muted-foreground">{selectedVM.disk} GB allocated</p>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="details" className="space-y-4">
                  <div className="grid gap-4">
                    <div>
                      <h4 className="mb-2 text-sm font-medium">Compute Resources</h4>
                      <div className="space-y-2 border border-border bg-secondary p-4">
                        <div className="flex justify-between">
                          ...
                          <span className="font-mono text-sm">{selectedVM.cpu}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">RAM</span>
                          <span className="font-mono text-sm">{selectedVM.ram} GB</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">Disk</span>
                          <span className="font-mono text-sm">{selectedVM.disk} GB</span>
                        </div>
                      </div>
                    </div>
              ...
              <DialogFooter className="gap-2">
                ...
                <Button variant="outline" onClick={(e) => handleConnectVM(selectedVM, e)}>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete <span className="font-mono font-medium">{vmToDelete?.name}</span>? This
